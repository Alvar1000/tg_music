...
- **🧠 Тесты:**
  ...
  - *Угадай группу по обложке (Часть 1 и Часть 2)* — два теста по 15 обложек: фото альбома → варианты групп, разбор каждого ответа и итоговый счёт (на FSM).
...
handlers/
  ...
  tests.py               # выбор теста + движки зодиака, обложек и квеста
...
states/states.py         # FSM-состояния для теста по обложкам и квеста
content/                 # весь контент в JSON (правится без программиста)
  facts.json
  quiz_zodiac.json
  quiz_covers_1.json     # тест по обложкам, Часть 1 (15 вопросов)
  quiz_covers_2.json     # тест по обложкам, Часть 2 (15 вопросов)
  covers/                # 30 файлов обложек альбомов (фото к вопросам)
